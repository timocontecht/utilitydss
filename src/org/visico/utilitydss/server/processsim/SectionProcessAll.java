...
public class SectionProcessAll extends SimProcess
{
	...
	
	{
		...
		NUM_PIPE = pipes;							// TODO not used anymore
		...
		Bed_preparation = bed_preparation;			// height of bed preparation
		...
		if(this.PUT == 0)
		{
		   ...
		   for (int i=1; i<=this.NUM_Pipe; i++)
		   {
			   ...
			   if(myModel.getReplacement()) {
					   ...
					   sendMessage(msg_4);}
			   		sendTraceNote("Activity: " + getName() + " Remove pipe: " + start.toString() + 
			   				" End: " + myModel.presentTime().toString());
			   		myModel.excavators.takeBack(1);
			   }
			   ...
			   if(myModel.getFoundation()) {
			   		...
			   		if(myModel.getActivityMsg() == 3)
					   {ActivityMessage msg_5 = new ActivityMessage(myModel, this, start, "Foundation " + i, myModel.presentTime()) ;
					   sendMessage(msg_5);}
		   ...
		   // use flags after a certain amount of pipes that the housing crew can start.
		   ...
		   if(this.NUM_CONNECTIONS != 0)
		   {
			   ...
			   for (int j=1; j<=this.NUM_CONNECTIONS; j++) {
				   ...
				   if(myModel.getActivityMsgConnection() == 3 ){
				   		start = myModel.presentTime();
				   }
				   ...
				   if(myModel.getActivityMsgConnection() == 3 ){
				   		ActivityMessage msg_10 = new ActivityMessage(myModel, this, start, "Housing pipe " + j, myModel.presentTime()) ;
				   		sendMessage(msg_10);
				   }
				   ...
				   if(myModel.getActivityMsgConnection() == 3 ){
					   ActivityMessage msg_11 = new ActivityMessage(myModel, this, start, "Second Backfill " + j, myModel.presentTime());
				   	   sendMessage(msg_11);
				   }
				   ...
				   if(myModel.getActivityMsgConnection() == 2 ){
					   ActivityMessage msg_11 = new ActivityMessage(myModel, this, start, "Housing connection " + j, myModel.presentTime());
				   	   sendMessage(msg_11);
				   }
			   }
			   if(myModel.getActivityMsgConnection() == 1)
			   {
			   		ActivityMessage msg_10 = new ActivityMessage(myModel, this, start, "All Housing connections ", myModel.presentTime()) ;
			   		sendMessage(msg_10);
			   } 
			}
...
//=====================================================================================================================================================================
		...
		else
		{
		   ...
		   for (int i=1; i<=this.NUM_PIPE; i++)
		   {
			   ...
			   hold (new TimeSpan((myModel.getExcavatingTime() * (Excavation_volume/excavation)), TimeUnit.HOURS));
			   if(myModel.getActivityMsgPut() == 3)
			   		{ActivityMessage msg_2 = new ActivityMessage(myModel, this, start, "Excavate put " + i, myModel.presentTime()) ;
			   		sendMessage(msg_2);}
			   sendTraceNote("Activity: " + getName() + " Pipe: " + i + " Excavating Start: " + start.toString() + 
					   " End: " + myModel.presentTime().toString());
			   myModel.excavators.takeBack(1);
			   ...
			   if(myModel.getReplacement()) {
				   	...
				   	if(myModel.getActivityMsgPut() == 3)
					   {start = myModel.presentTime();}
				   	hold (new TimeSpan((myModel.getPutRemoveTime() * (1/put_removal)), TimeUnit.HOURS));
			   		if(myModel.getActivityMsgPut() == 3)
					   {ActivityMessage msg_4 = new ActivityMessage(myModel, this, start, "Remove Put " + i, myModel.presentTime()) ;
					   sendMessage(msg_4);}
			   		sendTraceNote("Activity: " + getName() + " Put removal: " + start.toString() + 
			   				" End: " + myModel.presentTime().toString());
			   		myModel.excavators.takeBack(1);
			   }
...
//=====================================================================================================================================================================
	...
	private int PUT; 
	private int NUM_PIPE;				// TODO not used anymore
